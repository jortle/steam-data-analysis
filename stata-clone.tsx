...
export default function StataClone() {
  ...
  const generateBarChart = () => {
    setOutput("Generating bar chart...")
    // In a real application, you'd generate the chart here
  }

  const calculateMean = () => {
    const mean = data.reduce((sum, item) => sum + item.income, 0) / data.length
    setOutput(`Mean income: $${mean.toFixed(2)}`)
  }
  ...
  const calculateMedian = () => {
    ...
    setOutput(`Median income: $${median.toFixed(2)}`)
  }

  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Stata Clone</h1>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <CardContent>
            <Tabs defaultValue="data">
              ...
              <TabsContent value="data">
                ...
                <Textarea value={JSON.stringify(data, null, 2)} readOnly className="w-full h-64" />
              </TabsContent>
        ...
        <Card>
          ...
          <CardContent>
            <Textarea value={output} readOnly className="w-full h-64" />
          </CardContent>
        </Card>
      </div>
    </div>
